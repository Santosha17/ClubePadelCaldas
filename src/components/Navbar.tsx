    ...
    return (
        <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-brand-navy py-4 shadow-xl' : 'bg-transparent py-6'}`}>
            <div className="container mx-auto px-6 flex justify-between items-center">
                ...
                {/* MENU DESKTOP (Estilo Great Padel) */}
                ...
                <div className="hidden md:block">
                    <button className="bg-brand-terracotta hover:bg-white hover:text-brand-navy text-white px-6 py-2.5 rounded-full font-bold text-xs md:text-sm tracking-wider transition-all duration-300 border-2 border-brand-terracotta shadow-lg transform hover:scale-105">
                        RESERVAR
                    </button>
                </div>
            ...
            {mobileMenuOpen && (
                <div className="fixed inset-0 bg-brand-navy z-40 flex flex-col items-center justify-center gap-8 text-white text-2xl font-bold animate-in fade-in zoom-in">
                    ...
                    <button className="bg-brand-terracotta px-10 py-3 rounded-full mt-4">RESERVAR</button>
                </div>
            )}
        </nav>
    );
